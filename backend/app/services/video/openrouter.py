...
class OpenRouterVideoService:
    ...
    def _build_payload(self, request: VideoRequest) -> dict:
        ...
        # -----------------------------------------
        # Kling Standard
        # Проверяем, поддерживает ли OpenRouter
        # официальный формат Kling.
        # -----------------------------------------

        if request.model.startswith("kwaivgi/kling-v3.0-std"):

            if request.start_frame_url:
                payload["image"] = request.start_frame_url

            if request.end_frame_url:
                payload["end_image"] = request.end_frame_url

        # -----------------------------------------
        # Все остальные модели OpenRouter
        # -----------------------------------------
